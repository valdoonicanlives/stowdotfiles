...
# have also set up an alias called SEARCHNOTES that cd's to the ~/notes dir and then calles the below function
fe() {
  local file
...
######=====================
yt2mp3() { yt-dlp -c --restrict-filenames --extract-audio --audio-quality 0 --audio-format mp3 -o "%(title)s.%(ext)s" $@ ;}
######===========
...
function qn() {
  echo $(date)'\n'$@ >> ~/text-files/qnotes.txt
}

function viewqn() {
	vim -c '$|startinsert!' ~/text-files/qnotes.txt
}
...
#######################################################
viewer () {'cd /home/delboy/mygits/nsxiv/ && nsxiv /home/delboy/Pictures' ;    }
